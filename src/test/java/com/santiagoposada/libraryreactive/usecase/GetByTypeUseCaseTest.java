...
class GetByTypeUseCaseTest {
    ...
    @Test
    void apply() {
        when(resourceRepository.findAllByType(resource.getType())).thenReturn(Flux.just(resource));
        when(resourceMapper.fromResourceEntityToDTO()).thenReturn(r->resourceDTO);


        Publisher<ResourceDTO> setup = getByTypeUseCase.apply(resource.getType());
        StepVerifier
                .create(setup)
                .expectNext(resourceDTO)
                .verifyComplete();

        verify(resourceRepository, times(1)).findAllByType(resource.getType());
    }
}