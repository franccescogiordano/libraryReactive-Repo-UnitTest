...
class GetByCategoryUseCaseTest {
    ...
    @Test
    void apply() {
        when(resourceRepository.findAllByCategory(resource.getCategory())).thenReturn(Flux.just(resource));
        when(resourceMapper.fromResourceEntityToDTO()).thenReturn(r->resourceDTO);


        Publisher<ResourceDTO> setup = getByCategoryUseCase.apply(resource.getCategory());
        StepVerifier
                .create(setup)
                .expectNext(resourceDTO)
                .verifyComplete();

        verify(resourceRepository, times(1)).findAllByCategory(resource.getCategory());
    }
}